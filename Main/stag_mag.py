        ...
        for ny in range(Ly):
            if nx%2 == 0:
                if ny%2 ==0:
                    ...
                    A_sites.append(coord_to_site_bravais(Lx,nx,ny))
                else:
                    B_coords.append((nx,ny))
                    B_sites.append(coord_to_site_bravais(Lx,nx,ny))
            else:
                if ny%2 ==0:
                    B_coords.append((nx,ny))
                    B_sites.append(coord_to_site_bravais(Lx,nx,ny))
...
def calculate_stag_mag(Lx,Ly,samples):
    ...
    avg_stag_mag = np.mean(abs(stag_mags))
    var = np.var(stag_mags)
    return stag_mags,avg_stag_mag,var
